...
#import "GYBankCardIdentity.h"

@interface GYBankCardIdentity ()

@property (nonatomic, strong ) NSString *bankName;/**<银行名称(招商银行）*/

@property (nonatomic, strong ) NSString *asname;/**<银行简称（招商）*/

@property (nonatomic, strong ) NSString *bankcardName;/**<名称·卡种*/
...
@implementation GYBankCardIdentity

+(instancetype)createCardno:(NSString *)cardnumber
{
    
    GYBankCardIdentity *bankCard=[[GYBankCardIdentity alloc]init];
    
    NSString *bankCardName=[bankCard returnBankName:cardnumber];
    
    bankCard.bankcardName=bankCardName;
    bankCard.bankName=bankCardName;
    bankCard.asname=bankCardName;
    bankCard.asname=bankCardName;
    bankCard.cardtype=bankCardName;
...
-(void)setAsname:(NSString *)asname
{
    
    
    NSRange rang=[asname rangeOfString:@"银行"];
    if(rang.length<=0)
    {
      _asname=@"";
        return;
    ...
    _asname=[asname substringToIndex:rang.location];

    
}
-(void)setBankName:(NSString *)bankName
{
    NSRange rang=[bankName rangeOfString:@"·"];
    if(rang.length<=0)
    {
        _bankName=@"";
        return;
    }
    _bankName=[bankName substringToIndex:rang.location];
}


-(void)setCardtype:(NSString *)cardtype
{
    NSRange rang=[cardtype rangeOfString:@"·"];
    if(rang.length<=0)
    {
        _cardtype=@"";
        return;
    }
    _cardtype=[cardtype substringFromIndex:rang.location+rang.length];
}
